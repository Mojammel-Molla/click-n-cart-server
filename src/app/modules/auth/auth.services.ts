...
const login = async (payload: { email: string; password: string }) => {
  ...
  const token = generate_token(
    token_data,
    local_config.user_jwt_secret as string
  );
...
const reset_pass_from_db = async (
  token: string,
  ...
  const verified_token = verify_token(
    token,
    local_config.user_jwt_secret as Secret
  );
  if (!verified_token) {
...
const register_into_db = async (
  payload: { user: User; shop?: Shop },
  file: TFile
) => {
  ...
  const user_data: User = {
    ...payload.user,
    role: payload.user.role.toUpperCase() as UserRole,
  };
  ...
  if (user_data.role === UserRole.VENDOR) {
    ...
    if (!payload.shop) {
      throw new http_error(
        ...
        'Please provide shop information.'
      );
    }
